...
public class Node extends _Properties {
	...
	List<MovementListener> movementListeners = new ArrayList<MovementListener>();// 运动监听器
...
//	double angle = 0;// add
	...
	RaidaDirector director=new RaidaDirector();
	...
	public Node(double time, double speed, double direction) {
		this.time = time;
		this.speed = speed;
		// this.angel = angel;
	...
	public int getAngle() {
//		System.out.println(this.radia.toString());
	...
	public Node(Node model) {
		if (model != null) {
			...
			communicationRange = model.communicationRange;// 通信范围
			sensingRange = model.sensingRange;// 感应范围
			color = model.color;
		}
	}
	...
	/**
	 * Returns the abscissa//横坐标 of this node.
	 */
	public double getX() {
		return coords.x;
	}
	...
	/**
	 * Returns the ordinate//纵坐标 of this node.
	 */
	public double getY() {
		return coords.y;
	}
	...
	public void setColor(String color) {
		...
		setProperty("color", color); // Used for property notification属性通知警告标记吧~
	}
	 ...
	 * determines the distance up to which this node can <i>send</i> messages to
	 * other nodes.
	 */
	...
	/**
	 * Returns the model node corresponding to that name, all properties
	 * assigned to this virtual node will be given to further nodes created
	 * using this model name. If the requested model does not exist, it is
	 * created. FIXME
	 */
	...
	/**
	 * Create a new Node based on the specified model. 根据制定的模型，创建一个新的节点
	 * 
	 * @param modelName
	 * @return pouet
	 */
	...
	public static Node newInstanceOfModel(String modelName) {
		...
		try {
			...
			System.err.println("Problem of model instantiation..");
			return new Node();
		}
	}
	 ...
	 * 
	 * @param x
	 *            The abscissa of the new location.
	 * @param y
	 *            The ordinate of the new location.
	 */
	...
	public void wrapLocation() {
		...
		setLocation((coords.x + dim.width) % dim.width, (coords.y + dim.height)
				% dim.height);
	}
	 ...
	 * 
	 * @param dx
	 *            The abscissa component.
	 * @param dy
	 *            The ordinate component.
	 */
	...
	public void setDirection(Point2D p) {
		setDirection(Math.atan2(p.getX() - coords.x, -(p.getY() - coords.y))
				- Math.PI / 2);
	}
	...
	public void move(double distance) {
		translate(Math.cos(direction) * distance, Math.sin(direction)
				* distance);
	}
	 ...
	 * 
	 * @param n
	 *            The node at the opposite endpoint.
	 * @return The requested link, or <tt>null</tt> if no such link is found.
	 */
	...
	/**
	 * Returns a list containing all links for which this node is the source.
	...
	/**
	 * Returns a list containing all undirected links adjacent to this node. The
	 * returned list can be subsequently modified without effect on the
	 * topology.
	 */
	 ...
	 * 
	 * @param directed
	 *            <tt>true</tt> for directed, <tt>false</tt> for undirected. The
	 *            returned list can be subsequently modified without effect on
	 *            the topology.
	 */
	 ...
	 * directed link. The returned list can be subsequently modified without
	 * effect on the topology.
	 * 
	 * @return A list containing the neighbors, with possible duplicates when
	 *         several links come from a same neighbor.
	 */
	 ...
	 * adjacent directed link. The returned list can be subsequently modified
	 * without effect on the topology.
	 * 
	 * @return A list containing the neighbors, with possible duplicates when
	 *         several links go towards a same neighbor.
	 */
	...
	/**
	 * Returns a list containing every node located within the sensing range The
	 * returned list can be modified without side effect.
	 * 
	...
	/**
	 * Returns a list containing every node located at the opposite endpoint of
	 * an adjacent undirected links. The returned list can be subsequently
	 * modified without effect on the topology.
	 * 
	 * @return A list containing the neighbors, with possible duplicates when
	 *         several links are shared with a same neighbor.
	 */
	...
	/**
	 * Returns a list of messages representing the mailbox of this node. The
	 * mailbox can be usefull to scrutinize the arriving of new messages without
	 * using the MessageListener interface, or to clear previous messages (since
	 * all received messages are retained in the mailbox). The returned list
	 * must be considered as the original copy of the node's mailbox.
	 */
	...
	/**
	 * Sends a message from this node to the specified destination node. A
	 * <tt>null</tt> destination stands for the broadcast mode (that is, all the
	 * neighbors of this node will receive the message). The content of the
	 ...
	 * 
	 * @param dest
	 *            The destination node, or <tt>null</tt>.
	 * @param content
	 *            The object whose reference will be passed.
	 */
	...
	/**
	 * Same method as <tt>send()</tt>, but retries to send the message later if
	 * the link to the destination disappeared during transmission. (Does not
	 * work for <tt>null</tt> destinations.)
	 * 
	 * @param dest
	 *            The non-null destination.
	 * @param content
	 *            The message.
	 */
	...
	/**
	 * Registers the specified node listener to this node. The listener will be
	 * notified whenever an undirected link incident to this node appears or
	 * disappears.
	 * 
	 * @param listener
	 ...
	 *            The connectivity listener.
	 * @param directed
	 *            The type of link to be listened to.
	 */
	public void addConnectivityListener(ConnectivityListener listener,
			boolean directed) {
		if (directed)
	...
	/**
	 * Unregisters the specified node listener for this node. The listener will
	 * be removed from the list of 'undirected' listener.
	 * 
	 * @param listener
	 *            The connectivity listener.
	 */
	...
	/**
	 * Unregisters the specified connectivity listener for this node. Depending
	 * on the parameter <tt>directed</tt>, the listener will be removed from the
	 * list of 'directed' or 'undirected' listeners.
	 * 
	 * @param listener
	 *            The connectivity listener.
	 * @param directed
	 *            The type of link that was listened to.
	 */
	public void removeConnectivityListener(ConnectivityListener listener,
			boolean directed) {
		if (directed)
	...
	/**
	 * Registers the specified movement listener to this node. The listener will
	 * be notified every time the location of this node changes.
	 * 
	 * @param listener
	...
	/**
	 * Registers the specified message listener to this node. The listener will
	 * be notified every time a message is received at this node.
	 * 
	 * @param listener
	 ...
	 * 
	 * @param x
	 *            The abscissa of the point to which the distance is measured.
	 * @param y
	 *            The ordinate of the point to which the distance is measured.
	 */
	...
	protected void notifyNodeMoved() {
		LinkedHashSet<MovementListener> union = new LinkedHashSet<MovementListener>(
				movementListeners);
		if (topo != null)
