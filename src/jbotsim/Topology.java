...
public class Topology extends _Properties {
	List<ConnectivityListener> cxUndirectedListeners = new ArrayList<ConnectivityListener>();// Á¬½Ó¼àÌýÆ÷
	...
	Dimension dimensions = new Dimension(800, 600);// ³ß´ç
	...
	/**
	 * Adds the specified node to this topology. The location of the node in the
	 * topology will be its current inherent location (or <tt>(0,0)</tt> if no
	 * location was prealably given to it).
	 * 
	 * @param n
	...
	/**
	 * Adds a new node to this topology at the specified location. The node will
	 * be created using default settings FIXME
	 * 
	 * @param x
	 *            The abscissa of the location.
	 * @param y
	...
	/**
	 * Removes the specified node from this topology. All adjacent links will be
	...
	/**
	 * Adds the specified link to this topology. Calling this method makes sense
	 * only for wired links, since wireless links are automatically managed as
	 * per the nodes' communication ranges.
	 * 
	 * @param l
	...
	public void addLink(Link l, boolean silent) {
		if (l.type == Type.DIRECTED) {
			...
			if (l.destination.outLinks.containsKey(l.source)) {
				Link edge = new Link(l.source, l.destination,
						Link.Type.UNDIRECTED, l.mode);
				edges.add(edge);
				if (!silent)
	 ...
	 * 
	 * @param directed
	 *            <tt>true</tt> for directed links, <tt>false</tt> for
	 *            undirected links.
	 */
	public List<Link> getLinks(boolean directed) {
		return (directed) ? new ArrayList<Link>(arcs) : new ArrayList<Link>(
				edges);
	}
	 ...
	 * listener will be notified whenever a link of the specified type is added
	 * or removed.
	 * 
	 * @param listener
	 *            The listener to register.
	 * @param directed
	 *            The type of links to be listened (<tt>true</tt> for directed,
	 *            <tt>false</tt> for undirected).
	 */
	public void addConnectivityListener(ConnectivityListener listener,
			boolean directed) {
		if (directed)
	...
	/**
	 * Unregisters the specified connectivity listener from the listeners of the
	 * specified type.
	 * 
	 * @param listener
	 *            The listener to unregister.
	...
	/**
	 * Registers the specified movement listener to this topology. The listener
	 * will be notified every time the location of a node changes.
	 * 
	 * @param listener
	...
	protected void notifyLinkAdded(Link l) {
		...
		LinkedHashSet<ConnectivityListener> union = new LinkedHashSet<ConnectivityListener>(
				directed ? cxDirectedListeners : cxUndirectedListeners);
		union.addAll(directed ? l.source.cxDirectedListeners
				: l.source.cxUndirectedListeners);
		union.addAll(directed ? l.destination.cxDirectedListeners
				: l.destination.cxUndirectedListeners);
		for (ConnectivityListener cl : new ArrayList<ConnectivityListener>(
				union))
			cl.linkAdded(l);
	}
	...
	protected void notifyNodeAdded(Node node) {
		for (TopologyListener tl : new ArrayList<TopologyListener>(
				topologyListeners))
			tl.nodeAdded(node);
	}
	...
	protected void notifyNodeSelected(Node node) {
		for (SelectionListener tl : new ArrayList<SelectionListener>(
				selectionListeners))
	...
	void updateWirelessLink(Node n1, Node n2) {
		...
		if (l == null) {
			if (n1.distance(n2) < RangeEnum.SIXTH.getValue())
				addLink(new Link(n1, n2, Type.DIRECTED, Mode.WIRELESS));
		} else if (l.isWireless()
				&& n1.distance(n2) > RangeEnum.SIXTH.getValue())
			removeLink(l);
	...
	// double x2 = n2.getX();
	// double y2 = n2.getY();
	// double directionN1 = n1.getDirection();
	// double directionN1Jiaodu = (2 * Math.PI - directionN1) / Math.PI * 180;
	// double result = 0;
	// double angle = n1.getAngle();
	// double angle=90;

	// if (directionN1Jiaodu <= 180 && directionN1Jiaodu != 90) {
	// double s = (x2 - x1) + (Math.tan(directionN1Jiaodu)) * (y2 - y1);
	// double x = Math.sqrt((x2 - x1) * (x2 - x1)
	// * (Math.tan(directionN1Jiaodu))
	// * (Math.tan(directionN1Jiaodu)) + (y2 - y1) * (y2 - y1)
	// * (Math.tan(directionN1Jiaodu))
	// * (Math.tan(directionN1Jiaodu)));
	// result = Math.acos(s / x);
	// } else if (directionN1Jiaodu == 90) {
	// double s = y2 - y1;
	// double x = Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
	// result = Math.acos(s / x);
	// } else if (directionN1Jiaodu == 270) {
	// double s = y1 - y2;
	// double x = Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
	// result = Math.acos(s / x);
	// } else if (directionN1Jiaodu <= 360 && directionN1Jiaodu > 180
	// && directionN1Jiaodu != 270) {
	// double s = (x2 - x1) - (y2 - y1) * (Math.tan(directionN1Jiaodu));
	// double x = Math.sqrt((x2 - x1) * (x2 - x1)
	// * (Math.tan(directionN1Jiaodu))
	// * (Math.tan(directionN1Jiaodu)) + (y2 - y1) * (y2 - y1)
	// * (Math.tan(directionN1Jiaodu))
	...
	// n1.distance(n2)<=RangeEnum.SIXTH.getValue()) {
	// addLink(l);
	// if (n1.distance(n2) < RangeEnum.FIRST.getValue()) {
	//
	// } else if (n1.distance(n2) < RangeEnum.SECOND.getValue()) {
	//
	// } else if (n1.distance(n2) < RangeEnum.THIRD.getValue()) {
	//
	// } else if (n1.distance(n2) < RangeEnum.FOURTH.getValue()) {
	//
	// } else if (n1.distance(n2) < RangeEnum.FIFTH.getValue()) {
	//
	// } else if (n1.distance(n2) < RangeEnum.SIXTH.getValue()) {
	//
	// }
	// }
	// } else if (l.isWireless() && result > angle / 2) {
	// removeLink(l);
	// } else if (l.isWireless()
	// && n1.distance(n2) > RangeEnum.SIXTH.getValue()) {
	// removeLink(l);
	...
	// }
	//
	// } else if (l.isWireless() && n1.distance(n2) > n1.communicationRange)
	// removeLink(l);
	// }
	...
	/**
	 * Imports nodes and wired links from the specified string representation of
	 * a topology.
	 * 
	 * @param s
	...
	public void fromString(String s) {
		...
		while (s.indexOf("--") > 0) {
			...
			Node n2 = nodeTable.get(s.substring(s.indexOf(">") + 2,
					s.indexOf("\n")));
